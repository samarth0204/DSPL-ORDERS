...
import { Sun, Moon, CalendarIcon, Pencil, Trash2 } from "lucide-react";
...
const ReportCard: React.FC<ReportCardProps> = ({ entry }) => {
  const isLeaveDay = entry.EVENING?.remarks === "Leave Applied";
  return (
    <Card key={entry.date} className="shadow-sm border">
      ...
      <CardContent className="grid grid-cols-1 gap-4">
        {isLeaveDay ? (
          <div className="p-4 border rounded-md bg-yellow-50 text-center">
            <p className="text-yellow-800 font-semibold">
              Leave Applied for this day.
            </p>
          </div>
        ) : (
          <>
            ...
            <div className="p-4 border rounded-md bg-muted/30">
              ...
              {entry.EVENING ? (
                <div className="text-sm space-y-2">
                  ...
                  <div className="grid grid-cols-2">
                    <p>
                      <strong>CTC Weight:</strong>{" "}
                      {entry.EVENING.ctcWeight ?? "—"}
                    </p>
                    <p>
                  ...
                  <div className="grid grid-cols-2">
                    <p>
                      <strong>ATC Weight:</strong>{" "}
                      {entry.EVENING.atcWeight ?? "—"}
                    </p>
                    <p>
                      <strong>ATC Value:</strong>{" "}
                      {entry.EVENING.atcValue ?? "—"}
                    </p>
                  </div>
                  <Separator />
                  {/* VAP */}
                  <div className="grid grid-cols-2">
                    <p>
                      <strong>VAP Packet:</strong>{" "}
                      {entry.EVENING.vapPacket ?? "—"}
                    </p>
                    <p>
                      <strong>VAP Value:</strong>{" "}
                      {entry.EVENING.vapValue ?? "—"}
                    </p>
                  </div>
